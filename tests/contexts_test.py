...
class NextStateTableTest(absltest.TestCase):
  ...
  def test_invalid_args(self):
    with self.assertRaisesRegex(ValueError,
      ...
      contexts.NextStateTable(jnp.zeros([1, 0], dtype=jnp.int32))
    with self.assertRaisesRegex(ValueError,
                                'next_state_table should have a non-zero size'):
      contexts.NextStateTable(jnp.zeros([0, 1], dtype=jnp.int32))
    with self.assertRaisesRegex(ValueError,
                                'next_state_table should have shape'):
      contexts.NextStateTable(jnp.zeros([1], dtype=jnp.int32))
    with self.assertRaisesRegex(ValueError,
                                'next_state_table should be an int32 ndarray'):
      contexts.NextStateTable(jnp.zeros([2, 3]))
  ...
  def test_from_full_n_gram(self):
    ...
    context = contexts.NextStateTable(next_state_table)
    ...
    with self.subTest('invalid inputs'):
      with self.assertRaisesRegex(ValueError,
                                  r'weights\.shape\[-2:\] should be \(13, 3\)'):
        context.forward_reduce(jnp.zeros([4, 3]), semirings.Real)
      with self.assertRaisesRegex(ValueError,
